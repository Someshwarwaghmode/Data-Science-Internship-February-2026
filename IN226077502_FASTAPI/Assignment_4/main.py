...
from fastapi import FastAPI,Query,responses,status,HTTPException,Response
from pydantic import BaseModel,Field
...
products = [
    ...
    {"id":4, "name":"Pen Set", "price":49, "category":"Stationnery", "in_stock":True}
]
...
def add_to_cart(
    ...
    quantity: int = Query(..., description="Product Quantity")
):
    product = find_product(product_id)
    ...
    if not product["in_stock"]:
        raise HTTPException(status_code=404, detail=f"{product["name"]} is out of stock")
...
def show_item():
    ...
    if not cart:
        return {"message": "Cart is empty", "item":[], "grand_total":0}
    
    grand_total = sum([p["subtotal"] for p in cart])
...
def remove_from_cart(product__id: int, response: Response):

    #find the product inside the cart
    for item in cart:
        if item["product_id"]== product__id:
            cart.remove(item)
